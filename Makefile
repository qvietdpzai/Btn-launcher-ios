...
$(error This platform is not currently supported for building BtnLauncher)
endif
...
BTN_BUNDLE_DIR      ?= $(OUTPUTDIR)/BtnLauncher.app
BTN_JRE8_DIR        ?= $(SOURCEDIR)/depends/java-8-openjdk
BTN_JRE17_DIR       ?= $(SOURCEDIR)/depends/java-17-openjdk
BTN_JRE21_DIR       ?= $(SOURCEDIR)/depends/java-21-openjdk
...
METHOD_PACKAGE = \
	...
	if [ '$(SLIMMED_ONLY)' = '0' ]; then \
		zip --symlinks -r $(OUTPUTDIR)/com.btnlauncher.ios-$(VERSION)-$(PLATFORM_NAME)$$IPA_SUFFIX Payload; \
	fi; \
	if [ '$(SLIMMED)' = '1' ] || [ '$(SLIMMED_ONLY)' = '1' ]; then \
		zip --symlinks -r $(OUTPUTDIR)/com.btnlauncher.ios.slimmed-$(VERSION)-$(PLATFORM_NAME)$$IPA_SUFFIX Payload --exclude='Payload/BtnLauncher.app/java_runtimes/*'; \
	fi
...
help:
	echo 'Makefile to compile BtnLauncher'
	echo ''
	echo 'Usage:'
	...
	echo '    make payload                        Makes Payload/BtnLauncher.app'
	echo '    make package                        Builds ipa of BtnLauncher'
...
java:
	echo '[BtnLauncher v$(VERSION)] java - start'
	$(MAKE) -C JavaApp -j$(JOBS) BOOTJDK=$(BOOTJDK)
	echo '[BtnLauncher v$(VERSION)] java - end'
...
assets:
	echo '[BtnLauncher v$(VERSION)] assets - start'
	if [ '$(IOS)' = '0' ] && [ '$(DETECTPLAT)' = 'Darwin' ]; then \
		mkdir -p $(WORKINGDIR)/BtnLauncher.app/Base.lproj; \
		...
		echo 'Due to the required tools not being available, you cannot compile the extras for BtnLauncher with an iOS device.'; \
	fi
	echo '[BtnLauncher v$(VERSION)] assets - end'

payload: native java jre assets
	echo '[BtnLauncher v$(VERSION)] payload - start'
	$(call METHOD_DIRCHECK,$(WORKINGDIR)/BtnLauncher.app/libs)
	$(call METHOD_DIRCHECK,$(WORKINGDIR)/BtnLauncher.app/libs_caciocavallo)
	$(call METHOD_DIRCHECK,$(WORKINGDIR)/BtnLauncher.app/libs_caciocavallo17)
	cp -R $(SOURCEDIR)/Natives/resources/en.lproj/LaunchScreen.storyboardc $(WORKINGDIR)/BtnLauncher.app/Base.lproj/ || exit 1
	cp -R $(SOURCEDIR)/Natives/resources/* $(WORKINGDIR)/BtnLauncher.app/ || exit 1
	cp $(WORKINGDIR)/*.dylib $(WORKINGDIR)/BtnLauncher.app/Frameworks/ || exit 1
	cp -R $(SOURCEDIR)/JavaApp/libs/others/* $(WORKINGDIR)/BtnLauncher.app/libs/ || exit 1
	cp $(SOURCEDIR)/JavaApp/build/*.jar $(WORKINGDIR)/BtnLauncher.app/libs/ || exit 1
	cp -R $(SOURCEDIR)/JavaApp/libs/caciocavallo/* $(WORKINGDIR)/BtnLauncher.app/libs_caciocavallo || exit 1
	cp -R $(SOURCEDIR)/JavaApp/libs/caciocavallo17/* $(WORKINGDIR)/BtnLauncher.app/libs_caciocavallo17 || exit 1
	$(call METHOD_DIRCHECK,$(OUTPUTDIR)/Payload)
	cp -R $(WORKINGDIR)/BtnLauncher.app $(OUTPUTDIR)/Payload
	if [ '$(SLIMMED_ONLY)' != '1' ]; then \
		cp -R $(OUTPUTDIR)/java_runtimes $(OUTPUTDIR)/Payload/BtnLauncher.app; \
	fi
	ldid -S $(OUTPUTDIR)/Payload/BtnLauncher.app; \
	if [ '$(TROLLSTORE_JIT_ENT)' == '1' ]; then \
		ldid -S$(SOURCEDIR)/entitlements.trollstore.xml $(OUTPUTDIR)/Payload/BtnLauncher.app/BtnLauncher; \
	elif [ '$(PLATFORM)' == '6' ]; then \
		ldid -S$(SOURCEDIR)/entitlements.codesign.xml $(OUTPUTDIR)/Payload/BtnLauncher.app/BtnLauncher; \
	else \
		ldid -S$(SOURCEDIR)/entitlements.sideload.xml $(OUTPUTDIR)/Payload/BtnLauncher.app/BtnLauncher; \
	fi
	chmod -R 755 $(OUTPUTDIR)/Payload
	if [ '$(PLATFORM)' != '2' ]; then \
		$(call METHOD_MACHO,$(OUTPUTDIR)/Payload/BtnLauncher.app,$(call METHOD_CHANGE_PLAT,$(PLATFORM),$$file)); \
		$(call METHOD_MACHO,$(OUTPUTDIR)/java_runtimes,$(call METHOD_CHANGE_PLAT,$(PLATFORM),$$file)); \
	fi
	echo '[BtnLauncher v$(VERSION)] payload - end'

deploy:
	echo '[BtnLauncher v$(VERSION)] deploy - start'
	cd $(OUTPUTDIR); \
	if [ '$(IOS)' = '1' ]; then \
		ldid -S $(WORKINGDIR)/BtnLauncher.app || exit 1; \
		ldid -S$(SOURCEDIR)/entitlements.trollstore.xml $(WORKINGDIR)/BtnLauncher.app/BtnLauncher || exit 1; \
		sudo mv $(WORKINGDIR)/*.dylib $(PREFIX)Applications/BtnLauncher.app/Frameworks/ || exit 1; \
		sudo mv $(WORKINGDIR)/BtnLauncher.app/BtnLauncher $(PREFIX)Applications/BtnLauncher.app/BtnLauncher || exit 1; \
		sudo mv $(SOURCEDIR)/JavaApp/build/*.jar $(PREFIX)Applications/BtnLauncher.app/libs/ || exit 1; \
		cd $(PREFIX)Applications/BtnLauncher.app/Frameworks || exit 1; \
		sudo chown -R 501:501 $(PREFIX)Applications/BtnLauncher.app/* || exit 1; \
		...
		if [ '$(PLATFORM)' != '2' ] || [ '$(TEAMID)' = '-1' ] || [ '$(SIGNING_TEAMID)' = '-1' ] || [ '$(PROVISIONING)' = '-1' ]; then \
			...
			$(call METHOD_PACKAGE); \
			if [ '$(SLIMMED_ONLY)' = '0' ]; then \
				open $(OUTPUTDIR)/com.btnlauncher.ios-$(VERSION)-$(PLATFORM_NAME).ipa; \
			else \
				open $(OUTPUTDIR)/com.btnlauncher.ios.slimmed-$(VERSION)-$(PLATFORM_NAME).ipa; \
			fi; \
		fi; \
	else \
		echo 'Device not supported for deploy recipe.'; \
	fi
	echo '[BtnLauncher v$(VERSION)] deploy - end'

package: payload
	echo '[BtnLauncher v$(VERSION)] package - start'
	if [ '$(TEAMID)' != '-1' ] && [ '$(SIGNING_TEAMID)' != '-1' ] && [ -f '$(PROVISIONING)' ] && [ '$(DETECTPLAT)' = 'Darwin' ]; then \
		printf '<?xml version="1.0" encoding="UTF-8"?>\n<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n<plist version="1.0">\n<dict>\n	<key>application-identifier</key>\n	<string>$(TEAMID).com.btnlauncher.ios</string>\n	<key>com.apple.developer.team-identifier</key>\n	<string>$(TEAMID)</string>\n	<key>get-task-allow</key>\n	<true/>\n	<key>keychain-access-groups</key>\n	<array>\n	<string>$(TEAMID).*</string>\n	<string>com.apple.token</string>\n	</array>\n</dict>\n</plist>' > entitlements.codesign.xml; \
		$(MAKE) codesign; \
	...
	echo '[BtnLauncher v$(VERSION)] package - end'
	
dsym: payload
	echo '[BtnLauncher v$(VERSION)] dsym - start'
	dsymutil --arch arm64 $(OUTPUTDIR)/Payload/BtnLauncher.app/BtnLauncher; \
	rm -rf $(OUTPUTDIR)/BtnLauncher.dSYM; \
	mv $(OUTPUTDIR)/Payload/BtnLauncher.app/BtnLauncher.dSYM $(OUTPUTDIR)/BtnLauncher.dSYM
	echo '[BtnLauncher v$(VERSION)] dsym - end'
	
codesign:
	echo '[BtnLauncher v$(VERSION)] codesign - start'
	cp '$(PROVISIONING)' $(OUTPUTDIR)/Payload/BtnLauncher.app/embedded.mobileprovision
	$(call METHOD_MACHO,$(OUTPUTDIR)/Payload/BtnLauncher.app,$(call METHOD_CODESIGN,$(SIGNING_TEAMID),$$file))
	$(call METHOD_MACHO,$(OUTPUTDIR)/java_runtimes,$(call METHOD_CODESIGN,$(SIGNING_TEAMID),$$file))
	echo '[BtnLauncher v$(VERSION)] codesign - end'
clean:
	echo '[BtnLauncher v$(VERSION)] clean - start'
	rm -rf $(WORKINGDIR)
	rm -rf JavaApp/build
	rm -rf $(OUTPUTDIR)
	echo '[BtnLauncher v$(VERSION)] clean - end'
