# BtnLauncher for iOS
[![Development build](https://github.com/BtnLauncherTeam/BtnLauncher_iOS/actions/workflows/development.yml/badge.svg?branch=main)](https://github.com/BtnLauncherTeam/BtnLauncher_iOS/actions/workflows/development.yml)
[![Crowdin](https://badges.crowdin.net/btnlauncher/localized.svg)](https://crowdin.com/project/btnlauncher)
[![Discord](https://img.shields.io/discord/724163890803638273.svg?label=&logo=discord&logoColor=ffffff&color=7389D8&labelColor=6A7EC2)](https://discord.gg/btnlauncher-724163890803638273)

## Important note

**BtnLauncher has been discontinued** and is no longer supported. Its successor is available [here](https://github.com/AngelAuraMC/Amethyst-iOS).


## Introduction
BtnLauncher is a Minecraft: Java Edition launcher for Android, iOS, and iPadOS, based off of zhuowei's [Boardwalk](https://github.com/zhuowei/Boardwalk) project.
...
This repository contains the code for our iOS and iPadOS port of BtnLauncher. Looking for [Android?](https://github.com/BtnLauncherTeam/BtnLauncher)

## Getting started with BtnLauncher
The [BtnLauncher Website](https://btnlauncherteam.github.io/INSTALL.html#ios) has extensive documentation on how to install, set up, and play! For those who wish to install quickly, here's the basics:
...
- iOS 17.x and iOS 18.x is supported. However, a computer is required. These methods will ultilized usage of pymobiledevice3. Python 3.11.(x) must be properly set up on your computer. For more information, please check out the official Wiki: https://btnlauncherteam.github.io/JIT.html#what-are-the-methods-to-enable-jit

### Setting up to sideload
BtnLauncher can be sideloaded in many ways. Our recommended solution is to install [TrollStore](https://github.com/opa334/TrollStore) if your iOS version supports it. Installing with TrollStore allows you to permenantly sign the application, automatically enable JIT, and increase memory limits.
...
- Signing services that do not use your UDID (and use distribution certificates) are not supported, as BtnLauncher requires capabilities they do not allow. However, if you do managed to gain access to a Development certificate, due to it having the necessary entitlement (being com.apple.security.get-task-allow) to attach a debugger to the running process (enabling JIT), you may use a Development certificate.
  
- Only install sideloading software and BtnLauncher from trusted sources. We are not responsible for any harm caused by using unofficial software.
...
### Installing BtnLauncher
#### Release build (TrollStore)
1. Download an IPA of BtnLauncher in [Releases](https://github.com/BtnLauncherTeam/BtnLauncher_iOS/releases).
2. Open the package in TrollStore using the share menu.

#### Release build (AltStore/SideStore trusted source)
1. Add `BtnLauncher Repository` from the Trusted Sources menu.
2. Tap `FREE` to begin installing.

#### Nightly builds
*These builds can contain game-breaking bugs. Use with caution.*
1. Download an IPA build of BtnLauncher in the [Actions tab](https://github.com/BtnLauncherTeam/BtnLauncher_iOS/actions).
2. Open the downloaded IPA in your sideloading app to install.

### Enabling JIT
BtnLauncher makes use of **just-in-time compilation**, or JIT, to provide usable speeds for the end user. JIT is not supported on iOS without the application being debugged, so workarounds are required to enable it. You can use this chart to determine the best solution for you and your setup.
...
## Contributors
BtnLauncher is amazing, and surprisingly stable, and it wouldn't be this way without the commmunity that helped and contribute to the project! Some notable names:
...
- [Caciocavallo](https://github.com/BtnLauncherTeam/caciocavallo): [GNU GPLv2 License](https://github.com/BtnLauncherTeam/caciocavallo/blob/master/LICENSE).
...
- [LWJGL3](https://github.com/BtnLauncherTeam/lwjgl3): [BSD-3 License](https://github.com/LWJGL/lwjgl3/blob/master/LICENSE.md).
- [LWJGLX](https://github.com/BtnLauncherTeam/lwjglx) (LWJGL2 API compatibility layer for LWJGL3): unknown license.
