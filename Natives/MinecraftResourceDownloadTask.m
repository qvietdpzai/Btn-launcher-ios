...
- (instancetype)init {
    ...
    //backgroundSessionConfigurationWithIdentifier:@"com.btnlauncher.ios.downloadtask"];
...
- (void)downloadVersionMetadata:(NSDictionary *)version success:(void (^)())success {
    ...
    NSString *path = [NSString stringWithFormat:@"%1$s/versions/%2$@/%2$@.json", getenv("BTN_GAME_DIR"), versionStr];
    ...
    void(^completionBlock)(void) = ^{
        ...
        if (self.metadata[@"inheritsFrom"]) {
            NSMutableDictionary *inheritsFromDict = parseJSONFromFile([NSString stringWithFormat:@"%1$s/versions/%2$@/%2$@.json", getenv("BTN_GAME_DIR"), self.metadata[@"inheritsFrom"]]);
            if (inheritsFromDict) {
    ...
    if (!version) {
        ...
        if (json[@"NSErrorObject"]) {
            ...
            path = [NSString stringWithFormat:@"%1$s/versions/%2$@/%2$@.json", getenv("BTN_GAME_DIR"), json[@"inheritsFrom"]];
        } else {
            completionBlock();
            return;
        }
...
- (void)downloadAssetMetadataWithSuccess:(void (^)())success {
    ...
    NSString *path = [@(getenv("BTN_GAME_DIR")) stringByAppendingPathComponent:name];
...
- (NSArray *)downloadClientLibraries {
    ...
    for (NSDictionary *library in self.metadata[@"libraries"]) {
        ...
        NSString *path = [NSString stringWithFormat:@"%s/libraries/%@", getenv("BTN_GAME_DIR"), artifact[@"path"]];
...
- (NSArray *)downloadClientAssets {
    ...
    for (NSString *name in assets[@"objects"]) {
        ...
        if ([assets[@"map_to_resources"] boolValue]) {
            path = [NSString stringWithFormat:@"%s/resources/%@", getenv("BTN_GAME_DIR"), name];
        } else {
            path = [NSString stringWithFormat:@"%s/assets/objects/%@", getenv("BTN_GAME_DIR"), pathname];
        }
...
- (void)downloadModpackFromAPI:(ModpackAPI *)api detail:(NSDictionary *)modDetail atIndex:(NSUInteger)selectedVersion {
    ...
    NSURLSessionDownloadTask *task = [self createDownloadTask:url size:size sha:sha altName:nil toPath:packagePath success:^{
        NSString *path = [NSString stringWithFormat:@"%s/custom_gamedir/%@", getenv("BTN_GAME_DIR"), name];
