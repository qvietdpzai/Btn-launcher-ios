...
- (void)viewDidLoad
{
    ...
    NSString *path = [NSString stringWithFormat:@"%s/instances", getenv("BTN_HOME")];
...
- (void)changeSelectionTo:(NSString *)name {
    ...
    NSString *multidirPath = [NSString stringWithFormat:@"%s/instances/%@", getenv("BTN_HOME"), name];
    NSString *lasmPath = @(getenv("BTN_GAME_DIR"));
...
- (UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *directory = [NSString stringWithFormat:@"%s/instances/%@", getenv("BTN_HOME"), self.array[indexPath.row]];
...
- (void)textFieldDidEndEditing:(UITextField *)sender {
    ...
    NSString *dest = [NSString stringWithFormat:@"%s/instances/%@", getenv("BTN_HOME"), sender.text];
    if (isFooterView) {
        [NSFileManager.defaultManager createDirectoryAtPath:dest withIntermediateDirectories:NO attributes:nil error:&error];
    } else {
        NSString *source = [NSString stringWithFormat:@"%s/instances/%@", getenv("BTN_HOME"), sender.placeholder];
