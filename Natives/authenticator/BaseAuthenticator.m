...
+ (id)loadSavedName:(NSString *)name {
    NSMutableDictionary *authData = parseJSONFromFile([NSString stringWithFormat:@"%s/accounts/%@.json", getenv("BTN_HOME"), name]);
...
- (BOOL)saveChanges {
    ...
    NSString *newPath = [NSString stringWithFormat:@"%s/accounts/%@.json", getenv("BTN_HOME"), self.authData[@"username"]];
    if (self.authData[@"oldusername"] != nil && ![self.authData[@"username"] isEqualToString:self.authData[@"oldusername"]]) {
        NSString *oldPath = [NSString stringWithFormat:@"%s/accounts/%@.json", getenv("BTN_HOME"), self.authData[@"oldusername"]];
