...
void* btnGetCurrentContext() {
    return br_get_current();
}

int btnInit(BOOL useStackQueue) {
...
int btnInitOpenGL() {
    NSString *renderer = NSProcessInfo.processInfo.environment[@"BTN_RENDERER"];
    ...
    if (isAuto || [renderer isEqualToString:@ RENDERER_NAME_GL4ES]) {
        ...
        setenv("BTN_RENDERER", renderer.UTF8String, 1);
        set_gl_bridge_tbl();
...
void btnSetWindowHint(int hint, int value) {
    if (hint == GLFW_CLIENT_API) {
        clientAPI = value;
    } else if (strcmp(getenv("BTN_RENDERER"), "auto")==0 && hint == GLFW_CONTEXT_VERSION_MAJOR) {
        switch (value) {
            case 1:
            case 2:
                setenv("BTN_RENDERER", RENDERER_NAME_GL4ES, 1);
                JNI_LWJGL_changeRenderer(RENDERER_NAME_GL4ES);
                break;
            // case 4: use Zink?
            default:
                setenv("BTN_RENDERER", RENDERER_NAME_MTL_ANGLE, 1);
...
void btnMakeCurrent(basic_render_window_t* window) {
    br_make_current(window);
}

void* btnCreateContext(basic_render_window_t* contextSrc) {
...
void btnSwapInterval(int interval) {
