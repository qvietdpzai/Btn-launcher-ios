...
+ (void)tweakVersionJson:(NSMutableDictionary *)json {
    ...
    for (NSMutableDictionary *library in json[@"libraries"]) {
        ...
        if ([library[@"name"] hasPrefix:@"net.java.dev.jna:jna:"]) {
            ...
            // Early versions of the ASM library get repalced with 5.0.4 because BtnLauncher's LWJGL is compiled for
    ...
    NSDictionary *varArgMap = @{
        @"${classpath_separator}": @":",
        @"${library_directory}": [NSString stringWithFormat:@"%s/libraries", getenv("BTN_GAME_DIR")],
...
+ (NSObject *)findNearestVersion:(NSObject *)version expectedType:(int)type {
    ...
    if ([version isKindOfClass:NSString.class]){
        ...
        NSDictionary *versionDict = parseJSONFromFile([NSString stringWithFormat:@"%1$s/versions/%2$@/%2$@.json", getenv("BTN_GAME_DIR"), version]);
