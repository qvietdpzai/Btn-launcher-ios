...
int launchJVM(NSString *username, id launchTarget, int width, int height, int minVersion) {
    ...
    if ([launchTarget isKindOfClass:NSDictionary.class]) {
        ...
        gameDir = [NSString stringWithFormat:@"%s/instances/%@/%@",
            getenv("BTN_HOME"), getPrefObject(@"general.game_directory"),
            [PLProfiles resolveKeyForCurrentProfile:@"gameDir"]]
            .stringByStandardizingPath;
    } else {
        defaultJRETag = @"execute_jar";
        gameDir = @(getenv("BTN_GAME_DIR"));
        launchJar = YES;
    }
    ...
    if (javaHome == nil) {
        ...
        return 1;
    } else if ([javaHome hasPrefix:@(getenv("BTN_HOME"))]) {
    ...
    if (!launchJar) {
        margv[++margc] = "-Djava.system.class.loader=com.btnlauncher.BtnClassLoader";
    }
    margv[++margc] = "-Xms128M";
    ...
    margv[++margc] = [NSString stringWithFormat:@"-Duser.home=%s", getenv("BTN_HOME")].UTF8String;
    ...
    const char *glLibName = getenv("BTN_RENDERER");
    if (glLibName) {
    ...
    margv[++margc] = "com.btnlauncher.BtnLauncher";
