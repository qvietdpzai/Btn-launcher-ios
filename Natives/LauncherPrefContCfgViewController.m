...
- (void)loadGamepadConfigurationFile {
    NSString *gamepadPath = [NSString stringWithFormat:@"%s/controlmap/gamepads/%@", getenv("BTN_HOME"), getPrefObject(@"control.default_gamepad_ctrl")];
...
- (void)actionOpenFilePicker:(void (^)(NSString *name))handler {
    ...
    vc.listPath = [NSString stringWithFormat:@"%s/controlmap/gamepads", getenv("BTN_HOME")];
...
- (void)actionMenuSaveWithExit:(BOOL)exit {
    ...
    [controller addAction:[UIAlertAction actionWithTitle:localize(@"OK", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        BOOL success = [jsonData writeToFile:[NSString stringWithFormat:@"%s/controlmap/gamepads/%@.json", getenv("BTN_HOME"), field.text] options:NSDataWritingAtomic error:&error];
        if (!success) {
