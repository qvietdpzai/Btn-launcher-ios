...
- (void)actionMenuSaveWithExit:(BOOL)exit {
    ...
    [controller addAction:[UIAlertAction actionWithTitle:localize(@"OK", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        BOOL success = [jsonData writeToFile:[NSString stringWithFormat:@"%s/controlmap/%@.json", getenv("BTN_HOME"), field.text] options:NSDataWritingAtomic error:&error];
        if (!success) {
...
- (void)actionOpenFilePicker:(void (^)(NSString *name))handler {
    ...
    vc.listPath = [NSString stringWithFormat:@"%s/controlmap", getenv("BTN_HOME")];
