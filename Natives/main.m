...
void init_logDeviceAndVer(char *argument) {
    // BtnLauncher version
    NSLog(@"[Pre-Init] BtnLauncher INIT!");
    ...
    setenv("BTN_DETECTEDINST", type, 1);
...
void init_setupAccounts() {
    NSString *controlPath = [@(getenv("BTN_HOME")) stringByAppendingPathComponent:@"accounts"];
...
void init_setupCustomControls() {
    NSString *controlPath = [@(getenv("BTN_HOME")) stringByAppendingPathComponent:@"controlmap"];
    [fm createDirectoryAtPath:controlPath withIntermediateDirectories:NO attributes:nil error:nil];
...
void init_setupMultiDir() {
    ...
    const char *home = getenv("BTN_HOME");
    ...
    setenv("BTN_GAME_DIR", lasmPath.UTF8String, 1);
}

void init_setupResolvConf() {
    // Write known DNS servers to the config
    NSString *path = [NSString stringWithFormat:@"%s/resolv.conf", getenv("BTN_HOME")];
...
void init_setupHomeDirectory() {
    ...
    setenv("BTN_HOME", realpath(homeDir.UTF8String, NULL), 1);
}
...
int main(int argc, char *argv[]) {
    ...
    // If sandbox is disabled, W^X JIT can be enabled by BtnLauncher itself
