...
void toggleIsolatedPref(BOOL forceEnable) {
    if (!pref.instancePath) {
        pref.instancePath = [NSString stringWithFormat:@"%s/launcher_preferences.plist", getenv("BTN_GAME_DIR")];
    }
...
NSString* getSelectedJavaHome(NSString* defaultJRETag, int minVersion) {
    ...
    if ([selectedDir isEqualToString:@"internal"]) {
        ...
        selectedDir = [NSString stringWithFormat:@"%s/java_runtimes/%@", getenv("BTN_HOME"), selectedDir];
    }
