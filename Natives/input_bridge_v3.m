...
void CTCClipboard_nQuerySystemClipboard(JNIEnv *env, jclass clazz) {
    ...
    // From Java_com_btnlauncher_AWTInputBridge_nativeClipboardReceived
...
void btnPumpEvents(void* window) {
    CallbackBridge_nativeSetInputReady(YES);
    //__android_log_print(ANDROID_LOG_INFO, "input_bridge_v3", "btnPumpevents %d", eventCounter);
...
JNIEXPORT void JNICALL Java_com_btnlauncher_uikit_UIKit_updateMCGuiScale(JNIEnv* env, jclass clazz, jint scale) {
    guiScale = scale;
}
